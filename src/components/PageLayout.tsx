...
function HeaderActionButtons({
  ...
  const btnClass = `p-2 rounded-lg transition-colors ${
    dark ? "text-g-400 hover:bg-g-800" : "text-g-400 hover:bg-g-200"
  }`;

  return (
    <>
      <button onClick={toggle} className={btnClass} title={dark ? "Modo claro" : "Modo oscuro"}>
        {dark ? <SunIcon /> : <MoonIcon />}
      </button>
      <button onClick={toggleSound} className={btnClass} title={soundOn ? "Silenciar" : "Activar sonido"}>
      ...
      {showExit && (
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
            dark ? "text-err-500 hover:bg-g-800" : "text-err-600 hover:bg-err-50"
          }`}
        >
          <ExitIcon />
          Salir
      ...
      >
        ...
        <header className="relative z-10 flex items-center justify-between px-4 py-3">
          ...
          <div
            ...
            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none [&>*]:pointer-events-auto"
          />

          {/* Right: actions — inline on sm+, hamburger on mobile */}
          <div className="flex items-center gap-2">
            {/* Desktop actions */}
            ...
            <div className="sm:hidden">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" sideOffset={8}>
                  <DropdownMenuItem onClick={toggle} className="flex items-center gap-2.5 cursor-pointer">
                    {dark ? <SunIcon /> : <MoonIcon />}
                    <span>{dark ? "Modo claro" : "Modo oscuro"}</span>
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={toggleSound} className="flex items-center gap-2.5 cursor-pointer">
                    {soundOn ? <SoundOnIcon /> : <SoundOffIcon />}
                    <span>{soundOn ? "Silenciar" : "Activar sonido"}</span>
                  </DropdownMenuItem>
                  {showExit && (
                    <DropdownMenuItem onClick={() => setExitModalOpen(true)} className="flex items-center gap-2.5 cursor-pointer text-err-600">
                      <ExitIcon />
                      <span>Salir</span>
                    </DropdownMenuItem>
                  )}
